...
Load MRI and determine its coordinate system first. As shown below, 
...
Check if there is a mismatch between the image and the labels of axis. If it is correct, then enter "n" in the command window of Matlab.
...
Launch the interactive programm to realign the image to ac-pc system.
``` matlab
cfg           = [];
...
```
Then you need to identify four landmarks and press corresponding keys to assign coordinates to them. For more details, you can watch this [video](https://static-content.springer.com/esm/art%3A10.1038%2Fs41596-018-0009-6/MediaObjects/41596_2018_9_MOESM7_ESM.mp4).
...
Save the preprocessed anatomical MRI out to nii file.
``` matlab
cfg           = [];
...
The electrode labels can be loaded from the header or generated by yourself, which will appear as a to-do list during the interactive electrode placement activity.
``` matlab
