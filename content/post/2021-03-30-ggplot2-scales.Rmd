...
`guide_bins()` is suited to the situation when a continuous variable is binned and then mapped to an aesthetic that produces a legend, such as size, colour and fill. 
...
base + guides(size = guide_bins(show.limits = TRUE,
                                axis = FALSE))|
base + guides(size = guide_bins(axis.colour = "red",
...
The `guide_colorsteps()` guide is a version of `guide_colourbar()` appropriate for **binned color and fill scales**. Arguments mostly mirror those for `guide_colourbar()`. The additional arguments are as follows:
...
base <- ggplot(mpg, aes(displ, hwy, colour = cyl)) + 
      geom_point() + 
      scale_color_binned()
base2 <- ggplot(mpg, aes(cyl, displ, colour = hwy)) +
  geom_point(size = 2)

base + guides(colour = guide_coloursteps(show.limits = TRUE,
                                         ticks = TRUE)) |
base2 + guides(colour = guide_colourbar(barheight = unit(2, "cm"),
                                        ticks = FALSE))|
base2 + guides(colour = guide_colourbar(barwidth = unit(0.5, "cm"),
                                        nbin = 5))
```
...
## Manual scales
You can create new scale with the “manual” version of each: `scale_linetype_manual()`, `scale_shape_manual()`, `scale_colour_manual()`, etc. Valid aesthetic values are described in `vignette("ggplot2-specs")`.
